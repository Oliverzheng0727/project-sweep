...
public struct CleanupItem: Identifiable, Codable, Sendable, Hashable {
    ...
    /// Localize only titles generated by this app. User titles and filenames stay verbatim.
    /// The optional marker lives in existing metadata, so older encoded items remain compatible.
    public var displayTitle: String {
        switch metadata["generatedTitle"] {
        case "session":
            guard let sessionID, !sessionID.isEmpty else { return title }
            return AppText.format("会话 %@", String(sessionID.prefix(8)))
        case "cursorWorkspace":
            return AppText.string("未知格式的工作区会话存储")
        default:
            return title
        }
    }
