        ...
        Button(action: select) {
            VStack(spacing: 9) {
                ZStack(alignment: .bottomTrailing) {
                    Image(systemName: "folder.fill").font(.system(size: 48, weight: .light)).foregroundStyle(project.isAvailable ? Color.teal : Color.secondary)
                    if selected {
                        Image(systemName: "checkmark.circle.fill").font(.title3).symbolRenderingMode(.palette)
                            .foregroundStyle(.white, .teal).offset(x: 7, y: 3)
                    }
                ...
                .background(selected ? Color.teal.opacity(0.12) : Color.secondary.opacity(0.045), in: RoundedRectangle(cornerRadius: 15))
                .overlay(RoundedRectangle(cornerRadius: 15).strokeBorder(selected ? Color.teal.opacity(0.7) : Color.clear, lineWidth: 1.5))
