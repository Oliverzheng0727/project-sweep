            ...
            List(plan.entries) { entry in
                VStack(alignment: .leading, spacing: 7) {
                    HStack {
                        ...
                        Spacer()
                        Text(entry.removal.title).font(.caption.weight(.medium))
                    }
                    Text(entry.url.path).font(.caption).foregroundStyle(.secondary).textSelection(.enabled)
                    if let target = entry.linkTarget { Text("保留原文件：\(target)").font(.caption).textSelection(.enabled) }
                    Text(entry.impact).font(.callout)
                }.padding(.vertical, 8)
            }
