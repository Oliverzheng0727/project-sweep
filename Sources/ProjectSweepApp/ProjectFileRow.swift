    ...
    var body: some View {
        HStack(spacing: 9) {
            if tree {
                if row.hasChildren {
                    Button { state.toggleProjectExpansion(row) } label: {
                        ...
                        .accessibilityLabel(AppText.format(row.isExpanded ? "收起目录 %@" : "展开目录 %@", row.item.title))
                        .accessibilityValue(AppText.string(row.isExpanded ? "已展开" : "已收起"))
            ...
            if row.showsCheckbox(mode: state.mode) {
                Toggle(AppText.format("选择 %@", row.item.title), isOn: Binding(get: { state.selected.contains(row.id) }, set: { _ in state.toggleProjectSelection(row) }))
                    .labelsHidden().toggleStyle(.checkbox).disabled(!row.item.isSelectable || state.busy || state.executing)
                    .accessibilityLabel(AppText.format(row.isRoot && state.mode == .remove ? "移除整个项目：%@" : "清理选择：%@", row.item.title))
                    .help(row.item.isSelectable ? AppText.string("勾选加入清理清单") : AppText.string(row.item.reason))
            } else { Color.clear.frame(width: 18, height: 1).accessibilityHidden(true) }
                ...
                VStack(alignment: .leading, spacing: 5) {
                    HStack(spacing: 7) {
                        Text(row.item.title).font(row.isRoot ? .headline : .body.weight(.medium)).lineLimit(1)
                            .layoutPriority(1)
                        if row.isRoot { badge(AppText.string("项目根目录")) }
                        if row.isContext { badge(AppText.string("所在目录")) }
                    }
                    Text((row.isRoot && state.mode == .remove && !row.isContext ? AppText.string("移除整个项目 · ") : "") + AppText.string(row.explanation(mode: state.mode)))
            ...
            Button { state.inspect(row.item) } label: { Image(systemName: "info.circle").frame(width: 24, height: 28) }
                .buttonStyle(.borderless).accessibilityLabel(AppText.format("查看 %@ 的详情", row.item.title)).disabled(state.busy)
