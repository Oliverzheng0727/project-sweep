...
struct ReviewView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text("最终执行清单 · \(plan.items.count) 项").foregroundStyle(.secondary)
            HStack(spacing: 30) {
            ...
            if !expanded.isEmpty {
                Label("已纳入 \(expanded.count) 个关联项；共享数据与子会话必须一起处理。请核对下方“关联纳入”标记。", systemImage: "link")
            ...
            List {
                ForEach([CleanupAction.trash, .deleteSession], id: \.self) { action in
                    ...
                    if !members.isEmpty {
                        Section(action == .trash ? "文件 · 移入废纸篓，可尝试恢复" : "历史会话 · 永久删除，无备份") {
