...
**[下载 Project Sweep 0.5.0](https://github.com/Oliverzheng0727/project-sweep/releases/latest)** · [查看更新记录](CHANGELOG.md)
...
_截图来自 0.4.x，为无损 Retina PNG，使用生成的演示数据和不含个人信息的共享路径。0.5.0 将搜索和常用操作移到了原生工具栏。_
...
当前版本 **0.5.0**，面向 Apple Silicon、macOS 14 及以上，使用本机临时签名；尚未公证或用于商店发布。从源码构建后，可将 `dist/ProjectSweep-macOS-arm64.zip` 解压到本机应用程序目录运行。

构建同时生成 `dist/Project Sweep.app`。如果工作区在 iCloud 等同步目录中，同步服务可能为应用补写 Finder 属性并干扰签名校验；ZIP 在独立临时目录完成签名与归档，不受这类属性回写影响。

1. 在「项目库」选择或拖入项目总目录，例如桌面的 `Claude` 文件夹。可通过侧栏「添加项目库」保存多个位置，每个位置独立授权，并列在「项目位置」下。旧版保存的项目库自动迁移；缺失或无法访问的位置仍保留入口，可重试或重新连接。移除项目库入口只解除保存的连接，不删除文件；切换项目库会清除旧扫描与清理选择。首页只列出第一层项目文件夹，不深入扫描所有项目。
...
4. 点击文件行查看右侧详情，勾选框单独控制清理选择。可拖动详情分隔线调整宽度，或关闭详情为列表留出更多空间。详情提供系统 Quick Look、放大预览、Finder 定位和「始终保留」。Git 已跟踪文件、工具配置与技能（包括 `.agents/skills`）、保留项和包含这些内容的上层目录受到保护；取消个人保留不会解除系统保护。
5. 「移除整个项目」仅在项目根目录显示清理勾选框，以整个所选文件夹为单位移入废纸篓，包括源码和作品。内部条目用于查看，并提示会随项目一起移入废纸篓。在同一项目的「关联记录」页另外勾选相关会话，然后与项目文件一起查看最终清单。
6. 检查最终清单后执行，在「清理记录」按操作查看每项结果。可单独恢复文件，或查看本次操作的批量恢复清单；逐项展示原位置和废纸篓位置，会话单独说明不纳入恢复。同名冲突不会覆盖，其他文件可继续恢复，未恢复项分别报告。
...
侧栏使用原生导航，将功能页面与保存的项目位置分组。搜索、刷新、网格／列表切换和项目库操作集中在窗口工具栏；项目页缩短顶部信息区，详情栏宽度可调整，为文件树保留更多空间。
...
Project Sweep 0.5.0 已使用本机安装的 **Codex CLI 0.147.0** 完成实际协议验收：在独立临时 `CODEX_HOME` 中启动官方 app-server，创建两个测试线程，通过 `thread/delete` 删除其中一个，确认另一个仍可读取且被删线程无法再读取。测试没有调用模型，也没有访问或删除用户正常会话。
...
切换到「关联关系」，可只读查看 Claude Code 和 Codex 本次扫描发现的原目录与引用，每个来源保留 AI 名称、类型和完整路径。关系只依据当前扫描证据生成，浏览时不跟随链接、不读取未校验目标；缺少目标证据或来源检查不完整时明确说明。数量表示发现的来源条目，不代表已启用技能。
...
清理记录按每次操作分组，默认展开最近一组。可恢复单个文件，也可一起查看该组仍可恢复的文件；已恢复、未完成清理或缺少恢复信息的记录不会再次进入恢复清单。历史会话没有默认备份，会在确认页单独说明并排除。

恢复前重新检查记录与文件身份。原位置存在同名文件时保留原状，其余项目可以继续恢复；结果分别显示已恢复数量、未恢复数量和原因。旧版清理记录无需迁移。
...
- 保存的项目库位置及各自书签、手动断开的默认工具、保留规则、Codex 路径、网格/列表和主题偏好保存在本地设置中；扫描摘要只留在内存。没有账号、遥测、后台自动清理或外部服务。本次更新不迁移清理记录存储格式。
...
本版通过 **126 项核心测试（含实际 Codex CLI 协议验收）、69 项界面状态检查和 664 个英文词条检查**。所有删除与恢复测试使用隔离生成数据。
